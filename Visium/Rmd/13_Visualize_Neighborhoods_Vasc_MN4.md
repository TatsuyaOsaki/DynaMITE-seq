...
nns_all$vasc_plot_label <- 'Vasc_Neg_or_Low'

# set VascHigh_MN4Low_Neighbor and VascHigh_MN4High_Neighbor columns
nns_all$VascHigh_MN4Low_Neighbor <- 'Not_Neighbor'
nns_all$VascHigh_MN4High_Neighbor <- 'Not_Neighbor'

# Second layer: neighbors, colored by GSVA Enrichment
# get list of vasc_high spots. Use "id" column
vasc_high_id_list <- unique(nns_all$id[nns_all$vasc_label == 'vasc_high'])
...
for (curr_id in vasc_high_id_list) {
    ...
    # set the vasc_plot_label to Neighbor for the spot ids which are neighbors of the current center spot
    # Also set VascHigh_MN4Low_Neighbor and VascHigh_MN4High_Neighbor columns
    for (curr_neighbor_id in curr_neighbor_id_list) {
        nns_all$vasc_plot_label[nns_all$id == curr_neighbor_id] <- 'VascHigh_Neighbor'

        # if curr_id is a VascHigh_MN4Low or VascHigh_MN4High spot, set accordingly...
        if (nns_all$vasc_MN4_label[nns_all$id == curr_id] == 'vasc_high_MN4_low') {
            nns_all$VascHigh_MN4Low_Neighbor[nns_all$id == curr_neighbor_id] <- 'Neighbor'
        }
        if (nns_all$vasc_MN4_label[nns_all$id == curr_id] == 'vasc_high_MN4_high') {
            nns_all$VascHigh_MN4High_Neighbor[nns_all$id == curr_neighbor_id] <- 'Neighbor'
        }
    }
}
...
for (curr_id in vasc_high_id_list) {
    ...
    # if curr_id is a VascHigh_MN4Low or VascHigh_MN4High spot, set accordingly...
    if (nns_all$vasc_MN4_label[nns_all$id == curr_id] == 'vasc_high_MN4_low') {
        nns_all$vasc_plot_label[nns_all$id == curr_id] <- 'VascHigh_MN4Low'
    }
    if (nns_all$vasc_MN4_label[nns_all$id == curr_id] == 'vasc_high_MN4_high') {
        nns_all$vasc_plot_label[nns_all$id == curr_id] <- 'VascHigh_MN4High'
    }
}
...
<thead>
	<tr><th></th><th scope=col>Barcode</th><th scope=col>Tissue_Slice</th><th scope=col>orig.ident</th><th scope=col>nCount_Spatial</th><th scope=col>nFeature_Spatial</th><th scope=col>visium_round</th><th scope=col>nCount_SCT</th><th scope=col>nFeature_SCT</th><th scope=col>SCT_snn_res.0.8</th><th scope=col>seurat_clusters</th><th scope=col>slice_ident</th><th scope=col>vasc_summed_expr</th><th scope=col>vasc_label</th><th scope=col>Tumor_score</th><th scope=col>Tumor_label1</th><th scope=col>Tumor_label</th><th scope=col>MHCI_score</th><th scope=col>MHCI_label1</th><th scope=col>MHCI_label</th><th scope=col>Tumor_MHCI_label</th><th scope=col>Tumor_MHCI_label2</th><th scope=col>vasc_label1</th><th scope=col>Navin_annotations</th><th scope=col>Navin_annotations_simplified</th><th scope=col>Navin_Clusters1</th><th scope=col>Navin_Clusters</th><th scope=col>Navin_Clusters2</th><th scope=col>All_Immune</th><th scope=col>Angiogenesis</th><th scope=col>AyersIFNG</th><th scope=col>B_Cell</th><th scope=col>CD8_T_Cell</th><th scope=col>DC</th><th scope=col>Endothelial_Activation</th><th scope=col>Endothelial_Cell</th><th scope=col>Endothelial_Chemokines</th><th scope=col>Exhaustion</th><th scope=col>GOBP_LEUKOCYTE_ADHESION_TO_VASC</th><th scope=col>GOBP_LEUKOCYTE_MIGRATION</th><th scope=col>GOBP_POSITIVE_REGULATION_OF_LEUKOCYTE_MIGRATION</th><th scope=col>ICB_Targets</th><th scope=col>LEC</th><th scope=col>M1_Macrophage</th><th scope=col>M2_Macrophage</th><th scope=col>MN4_EC_Phenotype</th><th scope=col>MN4_EC_Phenotype_Top30</th><th scope=col>Macrophage</th><th scope=col>NK_Cell</th><th scope=col>Proliferation</th><th scope=col>T_Cell</th><th scope=col>T_Reg</th><th scope=col>Upregulated_by_2_3_CGAMP</th><th scope=col>Upregulated_by_2_3_CGAMP_IFNb_OVERLAP</th><th scope=col>MN4_EC_Phenotype_Label</th><th scope=col>MN4_EC_Percentile_Label</th><th scope=col>vasc_MN4_label</th><th scope=col>vasc_MN4_percentile_label</th><th scope=col>x</th><th scope=col>y</th><th scope=col>cell</th><th scope=col>id</th><th scope=col>nn1_id</th><th scope=col>nn2_id</th><th scope=col>nn3_id</th><th scope=col>nn4_id</th><th scope=col>nn5_id</th><th scope=col>nn6_id</th><th scope=col>num_adjacent_nns</th><th scope=col>nn_list</th><th scope=col>Macrophage_nn_1</th><th scope=col>Macrophage_nn_2</th><th scope=col>Macrophage_nn_3</th><th scope=col>Macrophage_nn_4</th><th scope=col>Macrophage_nn_5</th><th scope=col>Macrophage_nn_6</th><th scope=col>Macrophage_nn_mean</th><th scope=col>Macrophage_nn_vals</th><th scope=col>T_Cell_nn_1</th><th scope=col>T_Cell_nn_2</th><th scope=col>T_Cell_nn_3</th><th scope=col>T_Cell_nn_4</th><th scope=col>T_Cell_nn_5</th><th scope=col>T_Cell_nn_6</th><th scope=col>T_Cell_nn_mean</th><th scope=col>T_Cell_nn_vals</th><th scope=col>CD8_T_Cell_nn_1</th><th scope=col>CD8_T_Cell_nn_2</th><th scope=col>CD8_T_Cell_nn_3</th><th scope=col>CD8_T_Cell_nn_4</th><th scope=col>CD8_T_Cell_nn_5</th><th scope=col>CD8_T_Cell_nn_6</th><th scope=col>CD8_T_Cell_nn_mean</th><th scope=col>CD8_T_Cell_nn_vals</th><th scope=col>NK_Cell_nn_1</th><th scope=col>NK_Cell_nn_2</th><th scope=col>NK_Cell_nn_3</th><th scope=col>NK_Cell_nn_4</th><th scope=col>NK_Cell_nn_5</th><th scope=col>NK_Cell_nn_6</th><th scope=col>NK_Cell_nn_mean</th><th scope=col>NK_Cell_nn_vals</th><th scope=col>B_Cell_nn_1</th><th scope=col>B_Cell_nn_2</th><th scope=col>B_Cell_nn_3</th><th scope=col>B_Cell_nn_4</th><th scope=col>B_Cell_nn_5</th><th scope=col>B_Cell_nn_6</th><th scope=col>B_Cell_nn_mean</th><th scope=col>B_Cell_nn_vals</th><th scope=col>DC_nn_1</th><th scope=col>DC_nn_2</th><th scope=col>DC_nn_3</th><th scope=col>DC_nn_4</th><th scope=col>DC_nn_5</th><th scope=col>DC_nn_6</th><th scope=col>DC_nn_mean</th><th scope=col>DC_nn_vals</th><th scope=col>Endothelial_Cell_nn_1</th><th scope=col>Endothelial_Cell_nn_2</th><th scope=col>Endothelial_Cell_nn_3</th><th scope=col>Endothelial_Cell_nn_4</th><th scope=col>Endothelial_Cell_nn_5</th><th scope=col>Endothelial_Cell_nn_6</th><th scope=col>Endothelial_Cell_nn_mean</th><th scope=col>Endothelial_Cell_nn_vals</th><th scope=col>LEC_nn_1</th><th scope=col>LEC_nn_2</th><th scope=col>LEC_nn_3</th><th scope=col>LEC_nn_4</th><th scope=col>LEC_nn_5</th><th scope=col>LEC_nn_6</th><th scope=col>LEC_nn_mean</th><th scope=col>LEC_nn_vals</th><th scope=col>MN4_EC_Phenotype_nn_1</th><th scope=col>MN4_EC_Phenotype_nn_2</th><th scope=col>MN4_EC_Phenotype_nn_3</th><th scope=col>MN4_EC_Phenotype_nn_4</th><th scope=col>MN4_EC_Phenotype_nn_5</th><th scope=col>MN4_EC_Phenotype_nn_6</th><th scope=col>MN4_EC_Phenotype_nn_mean</th><th scope=col>MN4_EC_Phenotype_nn_vals</th><th scope=col>vasc_plot_label</th><th scope=col>VascHigh_MN4Low_Neighbor</th><th scope=col>VascHigh_MN4High_Neighbor</th></tr>
...
```R
# counts...
dplyr::count(nns_all, vasc_plot_label)
dplyr::count(nns_all, VascHigh_MN4Low_Neighbor)
dplyr::count(nns_all, VascHigh_MN4High_Neighbor)
```
...
<thead>
	<tr><th scope=col>vasc_plot_label</th><th scope=col>n</th></tr>
	<tr><th scope=col>&lt;chr&gt;</th><th scope=col>&lt;int&gt;</th></tr>
</thead>
<tbody>
	<tr><td>VascHigh_MN4High </td><td>2410</td></tr>
	<tr><td>VascHigh_MN4Low  </td><td> 759</td></tr>
	<tr><td>VascHigh_Neighbor</td><td>4209</td></tr>
	<tr><td>Vasc_Neg_or_Low  </td><td>2333</td></tr>
</tbody>
</table>
...
<thead>
	<tr><th scope=col>VascHigh_MN4Low_Neighbor</th><th scope=col>n</th></tr>
	<tr><th scope=col>&lt;chr&gt;</th><th scope=col>&lt;int&gt;</th></tr>
</thead>
<tbody>
	<tr><td>Neighbor    </td><td>2912</td></tr>
	<tr><td>Not_Neighbor</td><td>6799</td></tr>
</tbody>
</table>
...
<caption>A data.frame: 2 × 2</caption>
<thead>
	<tr><th scope=col>VascHigh_MN4High_Neighbor</th><th scope=col>n</th></tr>
	<tr><th scope=col>&lt;chr&gt;</th><th scope=col>&lt;int&gt;</th></tr>
</thead>
<tbody>
	<tr><td>Neighbor    </td><td>5579</td></tr>
	<tr><td>Not_Neighbor</td><td>4132</td></tr>
</tbody>
</table>



# Now we have labels for each spot. Now we need to collect the GSVA Enrichment of interest and plot that for the VascHigh_Neighbor spots. 
...
Idents(data) <- 'vasc_plot_label'
SpatialPlot(data, pt.size.factor=2.25, stroke=0.25)[[1]] + ggplot2::theme_minimal()
```
...
options(repr.plot.width=15, repr.plot.height=12)
Idents(data) <- 'vasc_plot_label'
SpatialPlot(data, pt.size.factor=2.25, stroke=0.25)[[2]] + ggplot2::theme_minimal()
```
...
options(repr.plot.width=15, repr.plot.height=12)
Idents(data) <- 'vasc_plot_label'
SpatialPlot(data, pt.size.factor=2.25, stroke=0.25)[[3]] + ggplot2::theme_minimal()
```
...
plot_reshaped_data <- function(temp_seu_obj, curr_enrichment) {
    ...
    #pre_plot <- SpatialPlot(temp_seu_obj, group.by = 'GSVA_Enrichment2', cols = plot_colors, pt.size.factor=2.25, stroke=0.25)
    ...
    # define filename
    png_filename_VA1 <- paste0("/immuno/ian/Projects/2025_02_Barbie_Visium_Round1_Revisions/Output/13_Visualize_Neighborhoods_vasc_MN4/Overall_Plots_Vasc_MN4/png/Overall_Plot_VA1_",curr_enrichment)
    pdf_filename_VA1 <- paste0("/immuno/ian/Projects/2025_02_Barbie_Visium_Round1_Revisions/Output/13_Visualize_Neighborhoods_vasc_MN4/Overall_Plots_Vasc_MN4/pdf/Overall_Plot_VA1_",curr_enrichment)
    ...
    # define filename
    png_filename_VB1 <- paste0("/immuno/ian/Projects/2025_02_Barbie_Visium_Round1_Revisions/Output/13_Visualize_Neighborhoods_vasc_MN4/Overall_Plots_Vasc_MN4/png/Overall_Plot_VB1_",curr_enrichment)
    pdf_filename_VB1 <- paste0("/immuno/ian/Projects/2025_02_Barbie_Visium_Round1_Revisions/Output/13_Visualize_Neighborhoods_vasc_MN4/Overall_Plots_Vasc_MN4/pdf/Overall_Plot_VB1_",curr_enrichment)
    ...
    # define filename
    png_filename_VA2 <- paste0("/immuno/ian/Projects/2025_02_Barbie_Visium_Round1_Revisions/Output/13_Visualize_Neighborhoods_vasc_MN4/Overall_Plots_Vasc_MN4/png/Overall_Plot_VA2_",curr_enrichment)
    pdf_filename_VA2 <- paste0("/immuno/ian/Projects/2025_02_Barbie_Visium_Round1_Revisions/Output/13_Visualize_Neighborhoods_vasc_MN4/Overall_Plots_Vasc_MN4/pdf/Overall_Plot_VA2_",curr_enrichment)
...
reshape_quantile_and_plot <- function(seu_obj, enrichment_list) {
    ...
    for (curr_enrichment in enrichment_list) {
        ...
        df_melt_sub$GSVA_Enrichment2[df_melt_sub$vasc_plot_label == 'Vasc_Neg_or_Low'] <- 111
        df_melt_sub$GSVA_Enrichment2[df_melt_sub$vasc_plot_label == 'VascHigh_MN4Low'] <- 222
        df_melt_sub$GSVA_Enrichment2[df_melt_sub$vasc_plot_label == 'VascHigh_MN4High'] <- 333
...
# Now let's do the same, but plot VascHigh_MN4Low and VascHigh_MN4High separately
...
```R
unique(data@meta.data$vasc_plot_label)
```
...
<ol class=list-inline><li>'VascHigh_MN4High'</li><li>'VascHigh_Neighbor'</li><li>'Vasc_Neg_or_Low'</li><li>'VascHigh_MN4Low'</li></ol>
...
plot_reshaped_data_2 <- function(temp_seu_obj, curr_enrichment, plot_group_name) {
    ...
    # define filename
    png_filename_VA1 <- paste0("/immuno/ian/Projects/2025_02_Barbie_Visium_Round1_Revisions/Output/13_Visualize_Neighborhoods_vasc_MN4/",plot_group_name,"/png/Overall_Plot_VA1_",curr_enrichment)
    pdf_filename_VA1 <- paste0("/immuno/ian/Projects/2025_02_Barbie_Visium_Round1_Revisions/Output/13_Visualize_Neighborhoods_vasc_MN4/",plot_group_name,"/pdf/Overall_Plot_VA1_",curr_enrichment)
    ...
    # VB
    # define filename
    png_filename_VB1 <- paste0("/immuno/ian/Projects/2025_02_Barbie_Visium_Round1_Revisions/Output/13_Visualize_Neighborhoods_vasc_MN4/",plot_group_name,"/png/Overall_Plot_VB1_",curr_enrichment)
    pdf_filename_VB1 <- paste0("/immuno/ian/Projects/2025_02_Barbie_Visium_Round1_Revisions/Output/13_Visualize_Neighborhoods_vasc_MN4/",plot_group_name,"/pdf/Overall_Plot_VB1_",curr_enrichment)
    ...
    # VA2
    # define filename
    png_filename_VA2 <- paste0("/immuno/ian/Projects/2025_02_Barbie_Visium_Round1_Revisions/Output/13_Visualize_Neighborhoods_vasc_MN4/",plot_group_name,"/png/Overall_Plot_VA2_",curr_enrichment)
    pdf_filename_VA2 <- paste0("/immuno/ian/Projects/2025_02_Barbie_Visium_Round1_Revisions/Output/13_Visualize_Neighborhoods_vasc_MN4/",plot_group_name,"/pdf/Overall_Plot_VA2_",curr_enrichment)
...
reshape_quantile_and_plot_2 <- function(seu_obj, enrichment_list) {
    ...
    for (curr_enrichment in enrichment_list) {
        ...
        df_melt_sub_MN4Low <- df_melt_sub
        df_melt_sub_MN4High <- df_melt_sub

        
        # MN4Low focus
        # Grey out MN4High spots if they aren't MN4Low neighbors
        df_melt_sub_MN4Low$GSVA_Enrichment2[(df_melt_sub_MN4Low$vasc_plot_label == 'VascHigh_MN4High') &
                                            (df_melt_sub_MN4Low$VascHigh_MN4Low_Neighbor == 'Not_Neighbor')] <- 111
        # Grey out VascHigh_Neighbor spots if they aren't MN4Low Neighbors
        df_melt_sub_MN4Low$GSVA_Enrichment2[(df_melt_sub_MN4Low$vasc_plot_label == 'VascHigh_Neighbor') &
                                             (df_melt_sub_MN4Low$VascHigh_MN4Low_Neighbor == 'Not_Neighbor')] <- 111
        # Keep MN4Low spots
        df_melt_sub_MN4Low$GSVA_Enrichment2[df_melt_sub_MN4Low$vasc_plot_label == 'VascHigh_MN4Low'] <- 222

        
        # MN4High focus
        # Grey out MN4Low spots if they aren't MN4High neighbors
        df_melt_sub_MN4High$GSVA_Enrichment2[(df_melt_sub_MN4High$vasc_plot_label == 'VascHigh_MN4Low') &
                                             (df_melt_sub_MN4High$VascHigh_MN4High_Neighbor == 'Not_Neighbor')] <- 111
        # Grey out VascHigh_Neighbor spots if they aren't MN4High Neighbors
        df_melt_sub_MN4High$GSVA_Enrichment2[(df_melt_sub_MN4High$vasc_plot_label == 'VascHigh_Neighbor') &
                                             (df_melt_sub_MN4High$VascHigh_MN4High_Neighbor == 'Not_Neighbor')] <- 111
        # Keep MN4High spots
        df_melt_sub_MN4High$GSVA_Enrichment2[df_melt_sub_MN4High$vasc_plot_label == 'VascHigh_MN4High'] <- 333

        # Make the GSVA_Enrichment2 column a factor so we can use it to plot...
        df_melt_sub_MN4Low$GSVA_Enrichment2 <- as.factor(df_melt_sub_MN4Low$GSVA_Enrichment2)
        df_melt_sub_MN4High$GSVA_Enrichment2 <- as.factor(df_melt_sub_MN4High$GSVA_Enrichment2)
        
        # Make copy of seurat object for plotting, using the reshaped metadata
        temp_seu_obj_MN4Low <- seu_obj
        temp_seu_obj_MN4High <- seu_obj
        df_melt_sub_MN4Low <- as.data.frame(df_melt_sub_MN4Low)
        df_melt_sub_MN4High <- as.data.frame(df_melt_sub_MN4High)
        rownames(df_melt_sub_MN4Low) <- df_melt_sub_MN4Low$Barcode
        rownames(df_melt_sub_MN4High) <- df_melt_sub_MN4High$Barcode
        temp_seu_obj_MN4Low@meta.data <- df_melt_sub_MN4Low
        temp_seu_obj_MN4High@meta.data <- df_melt_sub_MN4High
        ...
        # Now we can proceed with the plotting using our function from above
        plot_reshaped_data_2(temp_seu_obj_MN4Low, curr_enrichment, plot_group_name='VascHigh_MN4Low_only')
        plot_reshaped_data_2(temp_seu_obj_MN4High, curr_enrichment, plot_group_name='VascHigh_MN4High_only')
...
plot_reshaped_data_3 <- function(temp_seu_obj, curr_enrichment, curr_id) {
    ...
    Idents(temp_seu_obj) <- 'GSVA_Enrichment2'
    #pre_plot <- SpatialDimPlot(temp_seu_obj, cols = plot_colors)
    pre_plot <- SpatialPlot(temp_seu_obj, group.by = 'GSVA_Enrichment2', cols = plot_colors, pt.size.factor=2.6, image.alpha = 0.95, stroke = 0.3)
    plot <- pre_plot[[img_to_use]] + 
                labs(title=curr_enrichment) + NoLegend()
    plot

    # define filename
    png_filename <- paste0("/immuno/ian/Projects/2025_02_Barbie_Visium_Round1_Revisions/Output/13_Visualize_Neighborhoods_vasc_MN4/Individual_Spots_Vasc_MN4/png/",curr_id,"_",curr_enrichment)
    pdf_filename <- paste0("/immuno/ian/Projects/2025_02_Barbie_Visium_Round1_Revisions/Output/13_Visualize_Neighborhoods_vasc_MN4/Individual_Spots_Vasc_MN4/pdf/",curr_id,"_",curr_enrichment)
...
reshape_quantile_and_plot_3 <- function(seu_obj, enrichment_list) {
    ...
    for (curr_enrichment in enrichment_list) {
        ...
        # get list of vasc_high spots. Use "id" column
        vasc_high_id_list <- unique(df_melt_sub$id[df_melt_sub$vasc_label == 'vasc_high'])
        # for each id in vasc_high_id_list, look at its neighbor ids. then, go and set vasc_plot_label to VascHigh_Neighbor
        # and set Neighbor or Not_Neighbor in two other columns, VascHigh_MN4Low_Neighbor and VascHigh_MN4High_Neighbor
        ...
        for (curr_id in vasc_high_id_list) {
            ...
            df_melt_sub$GSVA_Enrichment2[(df_melt_sub$vasc_plot_label == 'VascHigh_MN4Low') &
                                         (df_melt_sub$id == curr_id)] <- 222
            df_melt_sub$GSVA_Enrichment2[(df_melt_sub$vasc_plot_label == 'VascHigh_MN4High') &
